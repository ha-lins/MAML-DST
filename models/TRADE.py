...
from models.DNDGRU import *
from models.DND import *


class TRADE(nn.Module):
    def __init__(self, hidden_size, lang, path, task, lr, dropout, slots, gating_dict, nb_train_vocab=0, lr_meta=0):
        ...
        self.meta_optimizer = optim.Adam(self.parameters(), lr=lr_meta)
        self.scheduler = lr_scheduler.ReduceLROnPlateau(self.optimizer, mode='max', factor=0.5, patience=1,
                                                        min_lr=0.0001, verbose=True)
        self.scheduler = lr_scheduler.ReduceLROnPlateau(self.meta_optimizer, mode='max', factor=0.5, patience=1,
                                                        min_lr=0.0001, verbose=True)
    ...
    def print_loss(self):
        ...
        print_loss_ptr = self.loss_ptr / self.print_every  # pointer-generator
    ...
    def get_emb(self, slot_temp):
        key_emb = torch.zeros(self.hidden_size)
        for i, slot in enumerate(slot_temp):
            #Domain embedding
            if slot.split("-")[0] in self.lang.word2index.keys():
                domain_w2idx = [self.lang.word2index[slot.split("-")[0]]]
                domain_w2idx = torch.tensor(domain_w2idx)
                if USE_CUDA: domain_w2idx = domain_w2idx.cuda()
                domain_emb = self.decoder.embedding(domain_w2idx)
            if slot.split("-")[1] in self.lang.word2index.keys():
                slot_w2idx = [self.lang.word2index[slot.split("-")[1]]]
                slot_w2idx = torch.tensor(slot_w2idx)
                if USE_CUDA: slot_w2idx = slot_w2idx.cuda()
                slot_emb = self.decoder.embedding(slot_w2idx)

                # Combine two embeddings as one query
                combined_emb = domain_emb + slot_emb
            key_emb += combined_emb
    ...
    # def optimize_meta(self, clip):
    #     self.loss_meta.backward()
    #     clip_norm = torch.nn.utils.clip_grad_norm_(self.parameters(), clip)
    ...
    def meta_evaluate(self, dev, matric_best, slot_temp, early_stop=None):
        # Set to not-training mode to disable dropout
        self.encoder.train(False)
        self.decoder.train(False)
        print("STARTING EVALUATION")
        ...
        return joint_acc_score, turn_acc_score_ptr
...
class Generator(nn.Module):
    def __init__(self, lang, shared_emb, vocab_size, hidden_size, dropout, slots, nb_gate):
        ...
        # self.gru = DNDGRU(hidden_size, hidden_size, hidden_size, DND, 5)
        # self.dnd = DND(2, hidden_size)
    ...
    def forward(self, batch_size, encoded_hidden, encoded_outputs, encoded_lens, story, max_res_len, target_batches,
        ...
        if args["parallel_decode"]:
            ...
            for wi in range(max_res_len):
                dec_state, hidden = self.gru(decoder_input.expand_as(hidden), hidden, wi)
