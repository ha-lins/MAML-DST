...
def loadData():
    data_url = "data/multi-woz-2.0/data.json"
    dataset_url = "https://www.repository.cam.ac.uk/bitstream/handle/1810/280608/MULTIWOZ2.zip?sequence=3&isAllowed=y"
    if not os.path.exists("data"):
        os.makedirs("data")
        os.makedirs("data/multi-woz-2.0")

    if not os.path.exists(data_url):
        print("Downloading and unzipping the MultiWOZ dataset")
        resp = urllib.request.urlopen(dataset_url)
        zip_ref = ZipFile(BytesIO(resp.read()))
        zip_ref.extractall("data/multi-woz-2.0")
        zip_ref.close()
        shutil.copy('data/multi-woz-2.0/MULTIWOZ2 2/data.json', 'data/multi-woz-2.0/')
        shutil.copy('data/multi-woz-2.0/MULTIWOZ2 2/valListFile.json', 'data/multi-woz-2.0/')
        shutil.copy('data/multi-woz-2.0/MULTIWOZ2 2/testListFile.json', 'data/multi-woz-2.0/')
        shutil.copy('data/multi-woz-2.0/MULTIWOZ2 2/dialogue_acts.json', 'data/multi-woz-2.0/')
...
def createData():
    ...
    fin1 = open('data/multi-woz-2.0/data.json', 'r')
    data = json.load(fin1)

    fin2 = open('data/multi-woz-2.0/dialogue_acts.json', 'r')
    ...
    # with open('data/multi-woz-2.0/woz2like_data.json', 'w') as outfile:
...
def divideData(data):
    ...
    testListFile = []
    fin = open('data/multi-woz-2.0/testListFile.json', 'r')
    for line in fin:
        testListFile.append(line[:-1])
    fin.close()

    valListFile = []
    fin = open('data/multi-woz-2.0/valListFile.json', 'r')
    for line in fin:
