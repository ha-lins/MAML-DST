...
def read_langs(file_name, gating_dict, SLOTS, dataset, lang, mem_lang, sequicity, training, max_line = None,\
               switch_exceptd = True, use_bert=False, seed=10):
    ...
    with open(file_name) as f:
        ...
        if training and dataset=="train" and args["data_ratio"]!=100:
            random.Random(seed).shuffle(dials)
        ...
        for dial_dict in dials:
            ...
            for ti, turn in enumerate(dial_dict["dialogue"]):
                ...
                if use_bert:
                    dialog_history += (turn["system_transcript"] + " ; " + turn["transcript"] + " ; " + "[SEP]")
                ...
                if dataset == "train" or dataset == "dev":
                    if args["except_domain"] != "":
                        ...
                        turn_belief_dict = OrderedDict([(k, v) for k, v in turn_belief_dict.items() for sourced in args["source_domain"] if sourced in k])
                else:
                    if args["except_domain"] != "":
...
def prepare_data_seq(training, task="dst", sequicity=0, batch_size=100, switch_exceptd=True, use_bert=False, seed=10):
    ...
    if training:
        pair_train, train_max_len, slot_train = read_langs(file_train, gating_dict, ALL_SLOTS, "train", lang, mem_lang, sequicity, training, switch_exceptd=switch_exceptd, use_bert=use_bert, seed=seed)
        train = get_seq(pair_train, lang, mem_lang, batch_size, True, sequicity)
        nb_train_vocab = lang.n_words
        pair_dev, dev_max_len, slot_dev = read_langs(file_dev, gating_dict, ALL_SLOTS, "dev", lang, mem_lang, sequicity, training, use_bert=use_bert, seed=seed)
        dev   = get_seq(pair_dev, lang, mem_lang, eval_batch, False, sequicity)
        pair_test, test_max_len, slot_test = read_langs(file_test, gating_dict, ALL_SLOTS, "test", lang, mem_lang, sequicity, training, use_bert=use_bert, seed=seed)
